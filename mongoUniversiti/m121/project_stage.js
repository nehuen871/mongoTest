...
db.movies.aggregate(pipe).itcount()

{"$project": { "_id": 0, "titlemovi": {$split: ["$title", " "]}}}

var pipe = [{"$project": { "_id": 0,"tamaño":{$size:{"titlemovi": {$split: ["$title", " "]}}}}}]


"tamaño":{$size:{"$titlemovi": {$split: ["$title", " "]}}}

"titlemovi": {$split: ["$title", " "]}
